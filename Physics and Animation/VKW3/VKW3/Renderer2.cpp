...
#include "Engine2.h"
#define CAMERA_SPEED 10.0f * (float)Engine2::deltaTime
#define CAMERA_OFFSET_FROM_CENTER 0.0f
#define CAMERA_VIEW_HEIGHT 0.0f
#define CAMERA_POSITION_X -4.0f
#define CAMERA_POSITION_Y -7.0f
#define CAMERA_POSITION_Z +20.0f

Renderer2* Renderer2::gRenderer = nullptr;
static LRESULT CALLBACK WndProc(HWND _hWnd, UINT _uMsg, WPARAM _wParam, LPARAM _lParam)
{
	...
	case WM_PAINT:
		...
		break;
	case WM_SIZE:
		if (Renderer2::gRenderer != nullptr && _wParam != SIZE_MINIMIZED)
		{
			if (Renderer2::gRenderer->resizing == true || _wParam == SIZE_MAXIMIZED || _wParam == SIZE_RESTORED)
			{
				RECT rect;
	...
	case WM_ENTERSIZEMOVE:
		Renderer2::gRenderer->resizing = true;
		break;
	case WM_EXITSIZEMOVE:
		Renderer2::gRenderer->resizing = false;
		break;
	}
...
void Renderer2::Init(RendererInitProperties _rendererInitProperties)
{
	// Timer
	{
	...
	currentMaxAnimationCount = _rendererInitProperties.maxAnimationCount;
	currentMaxModelMatrixCount = _rendererInitProperties.maxModelMatrixCount;
}

void Renderer2::Load(RendererDataProperties _rendererDataProperties)
{
	// Load Uniform Buffers
	{
		...
		{
			modelMatrices.resize(currentMaxModelMatrixCount);
			modelMatrices[0] = glm::translate(glm::mat4(), glm::vec3(0.0f, 0.0f, 0.0f));
		...
		{
			skeletons.resize(currentMaxAnimationCount);
	...
	// Load ShaderModules
	{
		shaderModules.resize(_rendererDataProperties.shaderModulesNames.size());
		for (size_t i = 0; i != _rendererDataProperties.shaderModulesNames.size(); ++i)
		{
			size_t fileSize = 0;
			char* buffer = nullptr;

			Loader::LoadShader(_rendererDataProperties.shaderModulesNames[i].filename, fileSize, &buffer);
			...
			shaderModules[i].stage = _rendererDataProperties.shaderModulesNames[i].stage;
			shaderModules[i].entryPointName = _rendererDataProperties.shaderModulesNames[i].entryPointName;
		}
	}

	// Load Textures
	{
		textures.resize(_rendererDataProperties.textureNames.size());
		for (size_t i = 0; i != _rendererDataProperties.textureNames.size(); ++i)
		{
			// loading
			...
			VkFormat imageFormat;
			{
				...
				Loader::LoadTGA(_rendererDataProperties.textureNames[i], width, height, size, type, data);
	...
	// Load Models
	{
		for (size_t i = 0; i != _rendererDataProperties.modelNames.size(); ++i)
		{
			Loader::Mesh mesh;
			Loader::LoadModelASSIMP(&mesh, nullptr, _rendererDataProperties.modelNames[i], aiProcess_GenNormals | aiProcess_CalcTangentSpace | aiProcess_JoinIdenticalVertices);
...
void Renderer2::Setup(RendererSetupProperties _rendererSetupProperties)
{
	...
	/// pipelines creation
	{
		// Shader State
		{
			pipelineShaderStagesCreateInfos.resize(_rendererSetupProperties.shaders.size());
			for (size_t i = 0; i != _rendererSetupProperties.shaders.size(); ++i)
			{
				pipelineShaderStagesCreateInfos[i].resize(_rendererSetupProperties.shaders[i].size());

				for (size_t j = 0; j != pipelineShaderStagesCreateInfos[i].size(); ++j)
				{
					size_t index = _rendererSetupProperties.shaders[i][j];
		...
		// Pipeline Info
		{
			graphicsPipelineCreateInfos.resize(_rendererSetupProperties.pipelines.size());
			for (size_t i = 0; i != graphicsPipelineCreateInfos.size(); ++i)
			{
				...
				graphicsPipelineCreateInfos[i].stageCount = (uint32_t)pipelineShaderStagesCreateInfos[_rendererSetupProperties.pipelines[i].shaderStateIndex].size();
				graphicsPipelineCreateInfos[i].pStages = pipelineShaderStagesCreateInfos[_rendererSetupProperties.pipelines[i].shaderStateIndex].data();
				graphicsPipelineCreateInfos[i].pVertexInputState = &pipelineVertexInputStateCreateInfos[_rendererSetupProperties.pipelines[i].vertexInputStateIndex];
				graphicsPipelineCreateInfos[i].pInputAssemblyState = &pipelineInputAssemblyStateCreateInfos[_rendererSetupProperties.pipelines[i].inputAssemblyStateIndex];
				graphicsPipelineCreateInfos[i].pTessellationState = nullptr;
				graphicsPipelineCreateInfos[i].pViewportState = &pipelineViewportStateCreateInfos[_rendererSetupProperties.pipelines[i].viewportStateIndex];
				graphicsPipelineCreateInfos[i].pRasterizationState = &pipelineRasterizationStateCreateInfos[_rendererSetupProperties.pipelines[i].rasterizationStateIndex];
				graphicsPipelineCreateInfos[i].pMultisampleState = &pipelineMultisampleStateCreateInfos[_rendererSetupProperties.pipelines[i].multisampleStateIndex];
				graphicsPipelineCreateInfos[i].pDepthStencilState = &pipelineDepthStencilStateCreateInfos[_rendererSetupProperties.pipelines[i].depthStencilStateIndex];
				graphicsPipelineCreateInfos[i].pColorBlendState = &pipelineColorBlendStateCreateInfos[_rendererSetupProperties.pipelines[i].colorBlendStateIndex];
...
void GetAnimatedSkeleton(std::array<aiMatrix4x4, 64>& _bones, Scene2::Skeleton _assimpAnimation, float AnimationTime)
{
...
void GetAnimatedSkeleton(std::array<aiMatrix4x4, 64>& _bones, Scene2::Skeleton _assimpAnimation, Scene2::AnimationModifier _modifier, float AnimationTime)
{
	std::array<aiMatrix4x4, 64> bones;
				...
				else
				{
					...
					else
					{
						...
						if (_modifier.modifiers[i].weight != 0.0f)
						{
							...
							aiQuaternion::Interpolate(rotation, end, start, _modifier.modifiers[i].weight);
							rotation.Normalize();
						}
					}
				}
...
void Renderer2::Render(Scene2 & _scene)
{
	// Prepare To Draw
	{
	...
	size_t currentRenderTransformIndex = 0;
	// Iterate every object, set data to be updated on the gpu, and record draw calls
	{
		std::vector<std::vector<size_t>> layerOfObjects;
		for (size_t i = 0; i != _scene.objects.size(); ++i)
		{
			if (_scene.objects[i]->parent > i && _scene.objects[i]->parent != -1) // must wait for the parent to be processed?
			{
				// find layer object belongs to
				size_t depth = 0;
				size_t currentObject = i;
				for (;;)
				{
					if (_scene.objects[currentObject]->parent != -1)
					{
						currentObject = _scene.objects[currentObject]->parent;
						++depth;
					}
					else
						break;
				...
				if (depth >= layerOfObjects.size())
					layerOfObjects.resize(depth + 1);

				layerOfObjects[depth].push_back(i); // store object in the layer it needs to be processed in
			}
			else
			{
				// all objects with no leftover dependencies are used here clearing dependencies of layer 0
				if (_scene.objects[i]->visible) /// do
				{
					Draw(currentRenderTransformIndex, *_scene.objects[i], _scene.objects, _scene.models, _scene.skeletons);
				}
			}
		}

		for (size_t i = 0; i != layerOfObjects.size(); ++i)
		{
			for (size_t j = 0; j != layerOfObjects[i].size(); ++j)
			{
				// each layer clears the dependency of the next layer
				if (_scene.objects[layerOfObjects[i][j]]->visible) /// do
				{
					Draw(currentRenderTransformIndex, *_scene.objects[i], _scene.objects, _scene.models, _scene.skeletons);
				}
			}
	...
	// Update Uniforms
	{
		...
		// model matrices
		{
			...
			VkU::UniformBuffer stagingBuffer;
			{
				VkBufferCreateInfo stagingBufferCreateInfo = VkU::GetVkBufferCreateInfo(currentRenderTransformIndex * sizeof(glm::mat4), VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
				VK_CHECK_RESULT("	", vkCreateBuffer(device.handle, &stagingBufferCreateInfo, nullptr, &stagingBuffer.handle), " - ", stagingBuffer.handle, " - vkCreateBuffer");
				...
				void* data;
				VK_CHECK_RESULT("	", vkMapMemory(device.handle, stagingBuffer.memory, 0, currentRenderTransformIndex * sizeof(glm::mat4), 0, &data), " - ", data, " - vkMapMemory");
				memcpy(data, modelMatrices.data(), currentRenderTransformIndex * sizeof(glm::mat4));
				vkUnmapMemory(device.handle, stagingBuffer.memory);
			}
			...
			copyRegion.srcOffset = 0;
			copyRegion.dstOffset = 0;
			copyRegion.size = currentRenderTransformIndex * sizeof(glm::mat4);
		...
		// Update
			{
				...
				std::array<aiMatrix4x4, 64> bones;
				GetAnimatedSkeleton(bones, _scene.skeletons[0], _scene.animationModifiers[0], _scene.objects[0]->animationTime);
...
/// PRIVATE
void Renderer2::Draw(size_t& _currentRenderTransformIndex, Scene2::Object& _object, std::vector<Scene2::Object*>& _objects, std::vector<Scene2::Model>& _models, std::vector<Scene2::Skeleton>& _skeletons)
{
	// set model matrix data
	if (_object.parent == -1)
		memcpy(&modelMatrices[_currentRenderTransformIndex], &_object.transform[0][0], sizeof(float) * 16);
	else if (_object.parentJointIndex == -1)
		memcpy(&modelMatrices[_currentRenderTransformIndex], &(_objects[_object.parent]->transform * _object.transform)[0][0], sizeof(float) * 16);
	else
	{
		glm::mat4 offset = glm::transpose(Loader::aiMatrix4x4ToGlmMat4(_skeletons[0].joints[_object.parentJointIndex].offset));
		memcpy(&modelMatrices[_currentRenderTransformIndex], &(_objects[_object.parent]->transform * skeletons[0].bones[_object.parentJointIndex] * _object.transform)[0][0], sizeof(float) * 16);
	}

	// Record draw call
	VkDeviceSize offset = 0;

	uint32_t pushConstantData[4] = { (uint32_t)_currentRenderTransformIndex, 1, 1, 1 };
	...
	vkCmdBindPipeline(renderCommandBuffers[swapchainImageIndex], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines[_models[_object.modelID].pipelineIndex]);
	...
	vkCmdBindVertexBuffers(renderCommandBuffers[swapchainImageIndex], 0, 1, &vertexBuffers[_models[_object.modelID].vertexBufferIndex].handle, &offset);
	vkCmdBindIndexBuffer(renderCommandBuffers[swapchainImageIndex], indexBuffers[_models[_object.modelID].indexBufferIndex].handle, 0, indexBuffers[_models[_object.modelID].indexBufferIndex].type);
	vkCmdDrawIndexed(renderCommandBuffers[swapchainImageIndex], indexBuffers[_models[_object.modelID].indexBufferIndex].count, 1, 0, 0, 1);

	++_currentRenderTransformIndex;
}
